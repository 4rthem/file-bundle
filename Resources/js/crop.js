var CROP = function () {
	return function (options) {
		...
		this.eles = {ele: undefined, container: undefined, img: undefined, overlay: undefined};
		this.img = undefined;
		this.imgInfo = {aw: 0, ah: 0, w: 0, h: 0, at: 0, al: 0, t: 0, l: 0, s: 1};
		this.init = function (e) {
			this.settings = {slider: options.slider};
			var t, n, r, i = this;
			n = $("<div />").attr({"class": "crop-container"}).css({width: e.width(), height: e.height()});
			t = $("<img />").attr({"class": "crop-img"}).css({zIndex: 5999, top: 0, left: 0});
			r = $("<div />").attr({"class": "crop-overlay"}).css({zIndex: 6e3});
			n.append(r);
			n.append(t);
			e.append(n);
			this.eles.ele = e;
			...
			r.bind(document.ontouchstart !== null ? "mousedown" : "touchstart", function (e) {
				var t = $(this), n = {x: e.pageX || e.originalEvent.pageX, y: e.pageY || e.originalEvent.pageY}, s = {x: t.parent().offset().left, y: t.parent().offset().top};
				e.preventDefault();
				$(document).bind(document.ontouchmove !== null ? "mousemove" : "touchmove", function (e) {
					if (e.pageX || typeof e.originalEvent.changedTouches[0] !== undefined) {
						var r = {x: e.pageX || e.originalEvent.changedTouches[0].pageX, y: e.pageY || e.originalEvent.changedTouches[0].pageY};
						if (parseInt(t.css("top")) == 0) {
							t.css({top: i.eles.ele.offset().top, left: i.eles.ele.offset().left})
						}
						i.imgMove({t: parseInt(t.css("top")) - (s.y - (n.y - r.y)), l: parseInt(t.css("left")) - (s.x - (n.x - r.x))});
						t.css({left: s.x - (n.x - r.x), top: s.y - (n.y - r.y)})
					}
				});
		...
		this.loadImg = function (e, d) {
			...
			this.eles.img.attr("src", e).load(function () {
				...
				if (!d.w) {
					n.s = 1;
				} else if (n.w * r.hw < n.h * r.wh)
					n.s = n.aw / d.w;
				else
					n.s = n.ah / d.h;

				n.l = d.l * n.s / (n.aw / n.w);
				n.t = d.t * n.s / (n.ah / n.h);
				t.imgResize();
				t.slider(n.s)
			})
		};
		this.imgSize = function () {
			var e = this.eles.img, t = {w: e.css("width", "")[0].width, h: e.css("height", "")[0].height},
				n = this.eles.container;
			var r = {wh: n.width() / n.height(), hw: n.height() / n.width()};
			this.imgInfo.aw = t.w;
			this.imgInfo.ah = t.h;
			if (t.w * r.hw < t.h * r.wh) {
				this.imgInfo.w = n.width() - padding * 2;
				this.imgInfo.h = this.imgInfo.w * (t.h / t.w);
			} else {
				this.imgInfo.h = n.height() - padding * 2;
				this.imgInfo.w = this.imgInfo.h * (t.w / t.h);
			}
			this.imgInfo.al = padding;
			this.imgInfo.at = padding;
			this.imgResize();
			return r;
		};
		this.imgResize = function (e) {
			...
			n.s = e || n.s;
			t.css({width: n.w * n.s, height: n.h * n.s});
			this.imgMove({t: -(n.h * (r - n.s)) / 2, l: -(n.w * (r - n.s)) / 2});
		};
		this.imgMove = function (e) {
			...
			if (i > padding) {
				i = padding;
				n.t = n.at == padding ? 0 : -padding
			} else if (i < -(n.h * n.s - (r.height() - padding))) {
				i = -(n.h * n.s - (r.height() - padding));
				n.t = n.at == padding ? n.h * n.s - (r.height() - padding * 2) : n.h * n.s - (r.height() - padding)
			}
			if (s > padding) {
				s = padding;
				n.l = n.al == padding ? 0 : -padding
			} else if (s < -(n.w * n.s - (r.width() - padding))) {
				s = -(n.w * n.s - (r.width() - padding));
				n.l = n.al == padding ? n.w * n.s - (r.width() - padding * 2) : n.w * n.s - (r.width() - padding)
			}
			t.css({top: i, left: s})
		};
		this.slider = function (start) {
			var e = this;
			start = start || 1;
			$(this.settings.slider).noUiSlider({range: [1, 4], start: start, step: .002, handles: 1, slide: function () {
				var t = $(this).val();
				e.imgResize(t)
			}})
		};
		this.coordinates = function () {
			var t = this,
				...
				s = i.attr("src"),
				e = {
					x: -(parseInt(i.css("left")) - padding) * (n.aw / (n.w * n.s)),
					y: -(parseInt(i.css("top")) - padding) * (n.ah / (n.h * n.s)),
					w: (r.width() - padding * 2) * (n.aw / (n.w * n.s)),
					h: (r.height() - padding * 2) * (n.ah / (n.h * n.s)),
					image: s
				};
...
(function (e, t) {
	...
	e.fn.noUiSlider = function (n) {
		...
		function s(n) {
			n.preventDefault();
			var r = 0 === n.type.indexOf("touch"), i = 0 === n.type.indexOf("mouse"), s = 0 === n.type.indexOf("pointer"), o, u, a = n;
		...
		function c(e, t, n) {
			var r = e.data("nui").options, i = e.data("nui").base.data("handles"), s = e.data("nui").style;
		...
		function h(n, r) {
			var s = n.data("nui").number, u = {target: n.data("nui").target, options: n.data("nui").options, handle: n, i: s};
			...
			if (!1 === r.to[s])return{val: function (e) {
		...
		function d(e) {
			var t = this.base, n = t.data("style"), i = e.x - this.startEvent.x, s = "left" === n ? t.width() : t.height();
		...
		function m(t) {
			var n = this.handle, r = n[0].gPct(n.data("nui").style);
			n.children().addClass(N[4]);
			o(x.move, S, d, {startEvent: t, position: r, base: this.base, target: this.target, handle: n});
			o(x.end, S, v, {base: this.base, target: this.target, handle: n});
		...
		function y(e) {
			if (!this.base.find("." + N[4]).length) {
				...
				var s = i.data("style");
				e = e["left" === s ? "x" : "y"];
				var o = "left" === s ? i.width() : i.height(), u = [], a = {left: i.offset().left, top: i.offset().top};
				for (t = 0; t < n.length; t++)u.push({left: n[t].offset().left, top: n[t].offset().top});
		...
		var E = ".nui", S = e(document), x = {start: "mousedown touchstart", move: "mousemove touchmove", end: "mouseup touchend"}, T = e.fn.val, N = "noUi-base noUi-origin noUi-handle noUi-input noUi-active noUi-state-tap noUi-target -lower -upper noUi-connect noUi-vertical noUi-horizontal noUi-background noUi-z-index".split(" "), C = [N[0]], k = [N[1]], L = [N[2]], A = {to: function (e, t) {
			...
			this.target.val([this.i ? null : this.val(), this.i ? this.val() : null], {trusted: !1})
		}, function (e) {
			e.stopPropagation()
		}];
		window.navigator.pointerEnabled ? x = {start: "pointerdown", move: "pointermove", end: "pointerup"} : window.navigator.msPointerEnabled && (x = {start: "MSPointerDown", move: "MSPointerMove", end: "MSPointerUp"});
		e.fn.val = function () {
		...
		return function (t) {
			return this.each(function (n, r) {
				r = e(r);
				r.addClass(N[6]);
				var i, s, u, l, p = e("<div/>").appendTo(r), d = [], v = [k.concat([N[1] + N[7]]), k.concat([N[1] + N[8]])], b = [L.concat([N[2] + N[7]]), L.concat([N[2] + N[8]])];
				t = e.extend({handles: 2, margin: 0, orientation: "horizontal"}, t) || {};
				t.serialization || (t.serialization = {to: [!1, !1], resolution: .01, mark: "."});
				f(t, r);
				t.S = t.serialization;
				...
				r.data({base: p, mark: t.S.mark, decimals: u});
				for (i = 0; i < t.handles; i++)l = e("<div><div/></div>").appendTo(p), l.addClass(v[i].join(" ")), l.children().addClass(b[i].join(" ")), o(x.start, l.children(), m, {base: p, target: r, handle: l}), o(x.end, l.children(), g, {base: p, target: r, handle: l}), l.data("nui", {target: r, decimals: u, options: t, base: p, style: s, number: i}).data("store", h(l, t.S)), l[0].gPct = a, d.push(l), c(l, A.to(t.range, t.start[i]));
				p.data({options: t, handles: d, style: s});
				r.data({handles: d});
				o(x.end, p, y, {base: p, target: r, handles: d})
			})
		}.call(this, n)
	}
})($)
